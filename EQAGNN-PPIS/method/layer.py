...
class GVP(nn.Module):
    ...
    :param in_dims: tuple (n_scalar, n_vector) [ if there are no vector features then in deim = (in_feat_dim,0)]
    :param out_dims: tuple (n_scalar, n_vector) [ if there are no vector features then in deim = (out_feat_dim,0)]
...
class GVPConv_att(MessagePassing):
    """
    Graph convolution / message passing with Geometric Vector Perceptrons.
    Takes in a graph with node and edge embeddings,
    and returns new node embeddings.
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        #print(x_s.shape,edge_index.shape)
        message = self.propagate(
            edge_index,
            s=x_s,
            v=x_v.contiguous().view(x_v.shape[0], x_v.shape[1] * 3),
    ...
    def aggregate(self, inputs, index):
        msgs = inputs
        n_nodes = self.h.shape[0]
        g = self.h.view(n_nodes, self.n_heads, self.n_hidden)
        ...
        e = e.squeeze(-1)
        adj_mat = self.adj.view(n_nodes, n_nodes, 1)
        ...
        for k in range(self.edge_index.shape[1]):
          ...
          #print(k,i,j)
          att_weights[k][0] = a[j][i]
        att_weights = torch.Tensor(att_weights).to(torch.device('cuda'))
