...
from .gvp_layer import LayerNorm, GVP, GVPConvLayer, GVPConv, _merge, tuple_cat, tuple_sum, tuple_index, GVPConvLayer_att
from .gvp_layer import _norm_no_nan, randn, _split, _VDropout, Dropout
...
class GVPGNNModel(torch.nn.Module):
    ...
    def __init__(
        ...
        # activations = (F.gelu, None)
        _DEFAULT_V_DIM = (s_dim, v_dim)
        _DEFAULT_E_DIM = (s_dim_edge, v_dim_edge) 

        # Node embedding
        self.emb_in = torch.nn.Embedding(in_dim, 1)#s_dim)
        ...
        self.layers = torch.nn.ModuleList(
            GVPConvLayer(
                _DEFAULT_V_DIM, (s_dim_edge+1, v_dim_edge), #_DEFAULT_E_DIM, 
    ...
    def forward(self, batch):
        ...
        h_V = torch.as_tensor(torch.Tensor(batch.atoms), dtype=torch.float32)#self.emb_in(batch.atoms).squeeze()#torch.Tensor(batch.atoms)#self.emb_in(batch.atoms)  # (n,) -> (n, d)batch.atoms#
        #print(h_V.shape,torch.Tensor(batch.atoms).squeeze().shape)#,batch.atoms)
        ...
        #print(lengths.shape)
        #print(h_V.shape,h_E[0].shape,h_E[1].shape)
        h_V = self.W_v(h_V)
        h_E = self.W_e(h_E)
        h_E = (torch.cat([h_E[0],cosine], dim=-1), h_E[1])
        #print(h_V[0].shape, h_V[1].shape,h_E[0].shape,h_E[1].shape)
        ...
        out = _merge(*h_V)
        #print(out.shape)
        #out = self.pool(gvp._merge(*h_V), batch.batch)  # (n, d) -> (batch_size, d) gvp._merge(*h_V)#
        #print(out.shape,batch.batch)
...
class GVP_AAMPModel(torch.nn.Module):
    """
    GVP-GNN model from "Equivariant Graph Neural Networks for 3D Macromolecular Structure".
    """
    def __init__(
