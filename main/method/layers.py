...
class GVPConv_att(MessagePassing):
    ...
    def __init__(
        ...
        self.linear = nn.Linear(62, self.n_hidden *self.n_heads, bias=False)
    ...
    def aggregate(self, inputs, index):
        ...
        g = self.linear(self.h).view(n_nodes, self.n_heads, self.n_hidden)
        ...
        e = e.squeeze(-1)
        adj_mat = self.adj.view(n_nodes, n_nodes, 1)#self.n_heads)
        ...
        idx = idx.T
        att_weights = attn_res[idx[:,0], idx[:,1]]
        att_weights = att_weights.unsqueeze(1)
        att_weights = torch.Tensor(att_weights)#.to(torch.device('cpu'))
